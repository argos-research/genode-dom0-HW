...
int Dom0_server::Networker::connect(Genode::Env&)
{
...
void Dom0_server::Networker::serve(Genode::Env& _env)
{
	int message = 0;
	while(true)
	{
...
ssize_t Dom0_server::Networker::receive_data(void* data, size_t size, int _target_socket)
{
	ssize_t result = 0;
	ssize_t position = 0;
...
ssize_t Dom0_server::Networker::receiveInt32_t(int32_t& data, int _target_socket)
{
	return receive_data(&data, sizeof(data), _target_socket);
}

// Send data from buffer data with size size to the socket.
ssize_t Dom0_server::Networker::send_data(void* data, size_t size, int _target_socket)
{
	ssize_t result = 0;
	ssize_t position = 0;
...
// convenience function
ssize_t Dom0_server::Networker::sendInt32_t(int32_t data, int _target_socket)
{
	return send_data(&data, sizeof(data), _target_socket);
}

void Dom0_server::Networker::disconnect()
{
...
void Dom0_server::send_profile(Genode::Dataspace_capability xmlDsCap)
{
	_networker.send_profile(xmlDsCap);	
}

void Dom0_server::Networker::send_profile(Genode::Dataspace_capability xmlDsCap)
{
	Genode::Region_map* rm = &_env.rm();
	char* xml = (char*)rm->attach(xmlDsCap);
	if(std::strlen(xml)>0)
...
Dom0_server::Networker::Networker(Genode::Env& env,Taskloader::Connection* taskloader, Parser::Connection* parser, Timer::Connection* _timer) :
	Thread_deprecated{"networker"},
	_env(env),
...
void Dom0_server::Networker::entry()
{
	while (true)
	{
		connect(_env);
